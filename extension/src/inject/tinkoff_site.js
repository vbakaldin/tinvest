...
async function getHistory(ticker) {
	...
	let response = await fetch('https://api.tinkoff.ru/trading/user/operations?sessionId=' + s_id, {
		method: 'POST',
		headers: {
    ...
    if (ticker_regexp !== null) {
        ...
        if (info.payload.hasEvents) {
			...
			if (info.payload.positionTinkoff) {
				...
				if (!currency) {console.log('currency not defined');}
			...
			if (info.payload.positionTinkoffIis) {
				...
				if (!currency) {console.log('currency not defined is IIS');};
				//console.log(currency);
